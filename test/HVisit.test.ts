...
const asyncTestVisit: AsyncVisitor<
	ITestDocElementsMap,
	keyof ITestDocElementsMap,
	{count: number; nodeIds: string[]}
> = async (doc, nodeType, nodeId, context) => {
	if (context) {
		context.count = context.count + 1;
		context.nodeIds.push(`${nodeType}:${nodeId}`);
	}
...
describe('HVisit tests', () => {
	test('Normal visit', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {context});
		expect(context.count).toEqual(4);
		expect(context.nodeIds).toEqual([
			'Root:root_1',
			'Node:node_1',
	...
	test('Normal visit - depth first', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			traversal: DocumentVisitTraversal.DEPTH_FIRST
		});
		expect(context.count).toEqual(4);
	...
	test('Skip node types visit', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			typesToVisit: ['Node']
	...
	test('Skip node types visit - depth first', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			traversal: DocumentVisitTraversal.DEPTH_FIRST,
	...
	test('Skip traversal of types', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			typesToTraverse: ['Node']
	...
	test('Skip traversal of types - depth first', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			typesToTraverse: ['Node'],
	...
	test('Visit subtree', async () => {
		...
		context.count = 0;
		context.nodeIds = [];
		await visitDocument(testDoc, asyncTestVisit, {
			context,
			startElement: {
				type: 'Root',
				_id: 'root_1'
	...
	test('Visit subtree - depth first', async () => {
		const context = {count: 0, nodeIds: []};
		await asyncVisitDocument(testDoc, asyncTestVisit, {
			context,
			traversal: DocumentVisitTraversal.DEPTH_FIRST,
