...
export interface IDeleteElement<
	MapsInterface,
	U extends
		keyof EntitiesMaps<MapsInterface> = keyof EntitiesMaps<MapsInterface>
> extends IReplayableElementCommand {
 ...
 * visits a HDocument in breadth or depth first format from the
 * document root.
 */
...
/**
 * Async Visitor interface for a visitor pattern that
 * visits a HDocument in breadth or depth first format from the
 * document root.
 */
...
export type MapsOfNormDoc<T> =
	T extends INormalizedDocument<infer MapsInterface, any>
		? MapsInterface
		: never;

export type UOfNormDoc<T> =
	T extends INormalizedDocument<any, infer U> ? U : never;
